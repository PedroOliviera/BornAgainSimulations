...
def get_sample_FAparticles(P):
    # Define materials
    material_FA = ba.MaterialBySLD("FA", 26.97456797961441e-6, 1.6973893657202161e-6)
    material_PS = ba.MaterialBySLD("PS", 9.92693615450786e-6,0.014378277171312681e-6) #PS-b-P2VP
    material_Si_Sub = ba.MaterialBySLD("Si Sub", 20.064086060037788e-6, 0.458038933708916e-6)
    ...
    #material_FA = ba.RefractiveMaterial("FA", 5.6E-06, 2.46904652E-09)
    ...
    PS_thickness = 262.363
    num_layers = 4
    layer_thickness = PS_thickness/ num_layers

    omega_order = 9*nm
    spacing = 40*nm
    FA_radius = 14*nm
    FA_ff = ba.Sphere(FA_radius)
    FA_particle = ba.Particle(material_FA, FA_ff)

    center = ( - FA_radius - layer_thickness / 2) 
    dist = 25

    layout_FA = ba.ParticleLayout()
    x = []
    y = []
    distr = ba.DistributionGaussian(center* nm, dist *nm)
    for parsample in distr.distributionSamples():
        FA_ff = ba.Sphere(FA_radius*nm)
        FA_particle = ba.Particle(material_FA, FA_ff)
        particle_position = R3(0*nm, 0*nm, parsample.value)
        FA_particle.translate(particle_position)
        layout_FA.addParticle(FA_particle, parsample.weight)
    ...
    layout_FA.setInterference(iff)
    layout_FA.setTotalParticleSurfaceDensity(P)

    #Define roughness
    transient = ba.ErfTransient()
    autocorr_PS = ba.SelfAffineFractalModel(6.795*nm, 0.7, 200*nm)
    autocorr_SiO2 = ba.SelfAffineFractalModel(0.847*nm, 0.7, 25*nm)
    autocorr_Si = ba.SelfAffineFractalModel(0.1*nm, 0.7, 25*nm)

    roughness_PS = ba.Roughness(autocorr_PS, transient)
    roughness_SiO2 = ba.Roughness(autocorr_SiO2, transient)
    roughness_Si = ba.Roughness(autocorr_Si, transient)

    # Define layers
    layer_vac = ba.Layer(material_Vacuum)
    layer_polymer_top = ba.Layer(material_PS, PS_thickness/ num_layers *nm, roughness_PS)
    layer_polymer_top.addLayout(layout_FA)
    layer_polymer_top.setNumberOfSlices(100)
    layer_polymer = ba.Layer(material_PS, PS_thickness/ num_layers *nm)
    layer_polymer.addLayout(layout_FA)
    layer_polymer.setNumberOfSlices(200)
    layer_SiO2 = ba.Layer(material_SiO2, 2*nm, roughness_SiO2)
    layer_Si = ba.Layer(material_Si_Sub, roughness_Si)

    # Define sample
    sample = ba.Sample()
    sample.addLayer(layer_vac)
    sample.addLayer(layer_polymer_top)
    for i in range(num_layers - 1): 
        sample.addLayer(layer_polymer)
...
def plotPSRadius():
    ...
    for i, (radius, label) in enumerate(zip(P_P2VP_radius, labels)):
        sample = get_sample_particles(radius)
...
if __name__ == '__main__':
    ...
    P_FA_particleDensity = [0.001, 0.0002, 0.0001, 0.00005]
    ...
    for i, (radius, label) in enumerate(zip(P_FA_particleDensity, labels)):
        sample = get_sample_FAparticles(radius)
        res = get_simulation(radius, sample).simulate()
