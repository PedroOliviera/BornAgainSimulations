...
exp_npz_file = "32_15deg.npz"     # saved with Q axes: [qy_min,qy_max,qz_min,qz_max]
alpha_i_deg  = 0.15
beamtime     = "feb"
...
def sample_radial_paracrystal_truncated(omega_nm=10,#6,
    ...
    for i in range(1):
        ...
        particle_FA = ba.Particle(material_FA, ba.Sphere(FA_radius))
        FA_vertical_shift = P2VP_radius_z - FA_radius
        particle_FA_position = R3(0*nm, 0*nm, FA_vertical_shift)
        ...
        interior_layout.addParticle(particle_CoreShell, 1)

        

        interior_layout.addParticle(particle_CoreShell, weight_K[i])
