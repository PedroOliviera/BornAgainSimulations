...
def sample_radial_paracrystal_truncated_with_roughness(omega_nm=0,#6,
                              damping_length_nm=0, domain_size = 0, variance = 0): #3.6e-4
    material_PS  = ba.RefractiveMaterial("PS",     2.51433698E-06, 2.353858E-09)
    ...
    diameter = np.mean([67.5, 66.7, 63.8, 66.6, 66.9, 66, 62.6, 67.6, 65.3])
    ...
    top = ba.Layer(ba.Vacuum())
    top.addLayout(surface_layout)
    polymer = ba.Layer(material_PS, 2*nm)
...
omega_nm_array = [2, 4, 6, 8, 10, 12, 16, 25]
variance_nm_array = [2, 4, 6, 8, 10, 12, 16, 25]
dampening_length_array = [200, 400, 1000, 2000, 10000, 20000]
domain_size_array = [200, 400, 1000, 2000, 10000, 20000]

#for omega in omega_nm_array:
#    save_label = '_omega_' + str(omega) 

#    sample = sample_radial_paracrystal_truncated_with_roughness(omega_nm=omega,
#                              damping_length_nm=0, domain_size = 20000 , variance = 0)
...
for variance in variance_nm_array:
    save_label = '_variance_' + str(variance) 

    sample = sample_radial_paracrystal_truncated_with_roughness(omega_nm=2,
                              damping_length_nm=0, domain_size = 20000, variance = variance)
    I_sim, extent_angles = func_simulation(sample)

    plot2D(exp_arr=exp_arr, I_sim=I_sim, extent_angles=extent_angles, zmax = 3.7e4, zmin = 25)
    plt.savefig('Monolayer_paracrystal_sim1_0p2deg' + save_label + '.png', dpi = 300)
    plt.savefig('Monolayer_paracrystal_sim1_0p2deg' + save_label + '.pdf', dpi = 300)
    plot_horizontal_slice_simple(alpha_cut_deg=0.25, exp_arr=exp_arr, exp_extent=exp_axes, sim_arr=I_sim,sim_extent=extent_angles)
    
    plt.savefig('Monolayer_paracrystal_sim1_0p2deg_lineprofile' + save_label + '.png', dpi = 300)
    plt.savefig('Monolayer_paracrystal_sim1_0p2deg_lineprofile' + save_label + '.pdf', dpi = 300)

    plt.savefig('Monolayer_paracrystal_sim1_0p2deg_vertical_lineprofile' + save_label + '.png', dpi = 300)
    plt.savefig('Monolayer_paracrystal_sim1_0p2deg_vertical_lineprofile' + save_label + '.pdf', dpi = 300)

    g.save_npz_data('Monolayer_paracrystal_sim1_0p2deg' + save_label + '.npz',I_sim, extent_angles)
    plt.close()

    

for dampening_length in dampening_length_array:
    save_label = '_dampening_length_' + str(dampening_length) 

    sample = sample_radial_paracrystal_truncated_with_roughness(omega_nm=2,
                              damping_length_nm=dampening_length, domain_size = 0, variance = 0)
    I_sim, extent_angles = func_simulation(sample)
...
for domain_size in dampening_length_array:
    save_label = '_domain_size_' + str(domain_size) 

    sample = sample_radial_paracrystal_truncated_with_roughness(omega_nm=2,
                              damping_length_nm=0, domain_size = domain_size, variance = 0)
    I_sim, extent_angles = func_simulation(sample)
