...
exp_npz_file = "4_15deg.npz"     # saved with Q axes: [qy_min,qy_max,qz_min,qz_max]
alpha_i_deg  = 0.15
beamtime     = "feb"
ROI_deg      = (0, 0, 0.6, 1.75)           # (phi_min, alpha_min, phi_max, alpha_max)
#ROI_deg      = (0, 0.1, 1.5, 0.2)
#ROI_deg      = (0, 0.13, 0.6, 0.18)
...
def sample_radial_paracrystal_truncated(omega_nm=8,#6,
    ...
    material_PS  = ba.RefractiveMaterial("PS",     2.51433698E-06, 0) 
    material_P2VP  = ba.RefractiveMaterial("P2VP", 2.49112645E-06, 0 ) # 2.29112645E-06, 2.58315258E-09
    material_Si_Sub = ba.RefractiveMaterial("Si Sub", 5.04383115E-06, 0) #7.644e-06
    material_SiO2 = ba.RefractiveMaterial("SiO2", 4.74631315E-06, 0)
    material_FA  = ba.RefractiveMaterial("PS",     6E-06, 0) 
    ...
    P2VP_radius_xy = 31/2  #*nm
    P2VP_radius_z = P2VP_radius_xy * 0.634482
    P2VP_radius_factor = 0.634482
    P2VP_spacing = P2VP_radius_xy * 2 * 1.29357


    interior_layout = ba.ParticleLayout()
    stdev = 1*nm
    distr = ba.DistributionGaussian(P2VP_radius_xy*2, stdev)

    for parsample in distr.distributionSamples():
        
        ff_P2VP = ba.Spheroid(parsample.value/2*nm, parsample.value/2 * P2VP_radius_factor*nm)
        particle_P2VP = ba.Particle(material_P2VP, ff_P2VP)

        vertical_shift = layer_thickness/2 - P2VP_radius_z*nm
    ...
    #interference function
    iff2 = ba.InterferenceRadialParacrystal(P2VP_spacing*nm, 150*nm)
    iff_pdf2 = ba.Profile1DGauss(6*nm)
    iff2.setProbabilityDistribution(iff_pdf2)
    iff2.setKappa(0.1)
    #Particle Layout
    interior_layout.setInterference(iff2)
    interior_layout.setTotalParticleSurfaceDensity(density_nm2)
...
def sample_radial_paracrystal_truncated_zParticleDisplacement(omega_nm=8,#6,
                              damping_length_nm=1000, density_nm2=3.6e-4): #3.6e-4
    material_PS  = ba.RefractiveMaterial("PS",     2.51433698E-06, 2.353858E-09) 
    material_P2VP  = ba.RefractiveMaterial("P2VP", 2.49112645E-06, 2.58315258E-09 ) # 2.29112645E-06, 2.58315258E-09
    ...
    interior_layout = ba.ParticleLayout()
    stdev = 1*nm
    diam_distr = ba.DistributionGaussian(P2VP_radius_xy*2, stdev)

    top_shift = 2*P2VP_radius_z*nm + 4*nm
    bot_shift = total_thickness - 4*nm
    z_dist_distr = ba.DistributionGate(top_shift, bot_shift)

    layout_array = [interior_layout for i in diam_distr.distributionSamples()]

    for i, (parsample, shift) in enumerate(zip(diam_distr.distributionSamples(), z_dist_distr.distributionSamples())):
        
        ff_P2VP = ba.Spheroid(parsample.value/2*nm, parsample.value/2 * P2VP_radius_factor*nm)
        particle_P2VP = ba.Particle(material_P2VP, ff_P2VP)

        
        particle_P2VP_position = R3(0*nm, 0, -1*shift.value)
        print(shift.value)
        particle_P2VP.translate(particle_P2VP_position)
        layout_array[i].addParticle(particle_P2VP, 0.001)
    ...
    for layout in layout_array:
        layout.setInterference(iff2)
        layout.setTotalParticleSurfaceDensity(density_nm2*1000)
    ...
    top = ba.Layer(ba.Vacuum())
    #top.addLayout(surface_layout)
    polymer1 = ba.Layer(material_PS, total_thickness)
    for layout in layout_array:
        polymer1.addLayout(layout)
