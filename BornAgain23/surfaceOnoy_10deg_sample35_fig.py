...
def sample_radial_paracrystal_truncated_with_Gaussian(omega_nm=6,#6,
                              damping_length_nm=400, density_nm2=3.6e-4): #3.6e-4
    material_PS  = ba.RefractiveMaterial("PS",     2.51433698E-06, 2.353858E-09)
    m_substrate = ba.RefractiveMaterial("Si Sub", 5.0e-6, 7.8e-8)
    ...
    #diam_K, height_K, weight_K, labels = h_r.summarize_pairs_kmedoids(dmin_nm, hsub_nm, K=num_samples, scale=True)
    #h_r.visualize_kmedoids(dmin_nm, hsub_nm, diam_K, height_K, labels, weight_rep=weight_K)
    #diam_K, height_K, weight_K = h_r.summarize_pairs_quantile_grid(dmin_nm, hsub_nm)
    #h_r.visualize_quantile_grid(dmin_nm, hsub_nm)
    #h_r.plt.show()
    ...
    distr_PS_diameter = ba.DistributionGaussian(PS_diameter_mean, PS_diameter_stdev)
    
    #form factor
    surface_layout = ba.ParticleLayout(); 
    for PS_diameter in distr_PS_diameter.distributionSamples():
        R = truncated_radius( (PS_height/PS_diameter_mean * PS_diameter.value), PS_diameter.value )
        b = 2*R - PS_height/PS_diameter_mean * PS_diameter.value
        ff_PS = ba.SphericalSegment(R* nm, 0.0*nm, b* nm)
        particle_PS= ba.Particle(material_PS, ff_PS)
        surface_layout.addParticle(particle_PS, PS_diameter.weight)
...
# ---------- SIMULATE ----------
sim = g.get_simulation_2D(sample_model=sample, detectorDistBeamtime=beamtime, angle=alpha_i_deg, beamIntensity=8e11, ROI_deg=ROI_deg, divergence=False, resolution=False, oneThread=False)

alpha_horizontal_lincut = 0.209
...
plot_horizontal_slice_simple(alpha_cut_deg=alpha_horizontal_lincut, exp_arr=exp_arr, exp_extent=exp_axes, sim_arr=I_sim,sim_extent=extent_angles, save_fname='horizontal_S35_fitted_10deg')
plot_vertical_slice_simple(phi_cut_deg=phi_vertical_lincut, exp_arr=exp_arr, exp_extent=exp_axes, sim_arr=I_sim,sim_extent=extent_angles, save_fname='vertical_S35_fitted_10deg')
...
plt.savefig(r"C:\BornAgainSimulations\data\sim-npz\GISAXS_surface_0p1deg.png", dpi=500)
plt.savefig(r"C:\BornAgainSimulations\data\sim-npz\GISAXS_surface_0p1deg.pdf", dpi=500)
