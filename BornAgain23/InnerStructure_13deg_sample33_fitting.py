...
def sample_radial_paracrystal_truncated(omega_nm_int, P2VP_radius_xy, P2VP_radius_z, P2VP_std_dev_radius, P2VP_std_dev_height, P2VP_spacing, kappa_int, delta):
    ...
    PS_std_dev_radius = 2.34180742 #0.1#2.34180742 
    PS_std_dev_height = 3# 9.1#3.000000008 
    ...
    for r_sample in distr_radius.distributionSamples():
        ...
        for h_sample in distr_height.distributionSamples():
            ...
            interior_layout.addParticle(particle_P2VP, total_weight*1)
    ...
    #Particle Layout
    surface_layout.setInterference(iff_top)
...
if __name__ == "__main__":
    ...
    sample = sample_radial_paracrystal_truncated(
        omega_nm_int=9, 
        P2VP_spacing = 40.61897,
        kappa_int = 0.35,
        P2VP_radius_xy=18.5,
        P2VP_radius_z=22,#10.172595,
        P2VP_std_dev_radius=1.75,
        P2VP_std_dev_height=2,
        delta=3 #.537
        )
    ...
    I_sim, extent_angles = run_simulation(sample, beamtime, alpha_i_deg, ROI_deg, intensity=7e11, background = 30, sim_2D = True)

    # 5. VISUALIZE
    
    #I_sim = scipy.ndimage.gaussian_filter(I_sim, sigma=[2.0, 1.5])

    plot_horizontal_slice_simple(alpha_cut_deg=alpha_horizontal_lincut, exp_arr=exp_arr, exp_extent=exp_axes, sim_arr=I_sim, sim_extent=extent_angles, save_fname='horizontal_S33_fitted_13deg')
    plot_vertical_slice_simple(phi_cut_deg=phi_vertical_lincut, exp_arr=exp_arr, exp_extent=exp_axes, sim_arr=I_sim, sim_extent=extent_angles, save_fname='vertical_S33_fitted_13deg')
    ...
    ax1.set_yticks(np.arange(0, 1.80, 0.25))
    ax1.set_xticks(np.arange(-0.5, 0.55, 0.25))
    plt.savefig(r"C:\Users\Pedro\OneDrive - McMaster University\PhD - School\Research\Projects\X Ray Scattering and Diffraction\Paper\Figures\GISAXS Fits\GISAXS_interior_0p13deg_sample33.png", dpi=500)
    plt.savefig(r"C:\Users\Pedro\OneDrive - McMaster University\PhD - School\Research\Projects\X Ray Scattering and Diffraction\Paper\Figures\GISAXS Fits\GISAXS_interior_0p13deg_sample33.pdf", dpi=500)
    plt.show()