...
def sample_radial_paracrystal_truncated(omega_nm=6,#6,
    ...
    material_PS = ba.MaterialBySLD("PS", delta*1e-6, beta*1e-6) #PS-b-P2VP
    material_Si_Sub = ba.MaterialBySLD("Si Sub", 20.07097738274431e-6, 0.4581962543112546e-6)
    material_SiO2 = ba.MaterialBySLD("SiO2", 18.865573446690597e-6, 0.2441926770268623e-6)
    material_Vacuum = ba.MaterialBySLD("Vacuum", 0.0, 0.0)

    total_thickness = 305 * nm
    offset = 20
    spacing = 63 - offset
    num_samples = 10
    ...
    for i in range(num_samples):
        R = truncated_radius(height_K[i] + 2, diam_K[i] - offset)
        b = 2*R - height_K[i] + 2
    ...
    #Layers
    top = ba.Layer(material_Vacuum)
    top.addLayout(surface_layout)
    polymer = ba.Layer(material_PS, total_thickness*nm)
    sub_oxide = ba.Layer(material_SiO2, 2.395*nm)
    ...
    s.addLayer(top)
    s.addLayer(polymer)
    s.addLayer(sub_oxide)
    s.addLayer(sub)
    return s

def sample_radial_paracrystal_old(omega_nm=10,#6,
    ...
    material_P2VP_2 = ba.RefractiveMaterial("P2VP_2", 3.96553077e-06, 1.04599986e-07)
    ...
    for xy_radius in distr_radius.distributionSamples():
        #ff_P2VP = ba.Sphere(xy_radius.value) 
        ff_P2VP = ba.Spheroid(xy_radius.value, (P2VP_radius_z/P2VP_radius_xy) * xy_radius.value) 
        particle_P2VP = ba.Particle(material_P2VP_2, ff_P2VP)
...
exp_npz_file = "33_10deg.npz"     # saved with Q axes: [qy_min,qy_max,qz_min,qz_max]
alpha_i_deg  = 0.10
beamtime     = "feb"
...
alpha_horizontal_lincut = 0.141
phi_vertical_lincut = 0.141
