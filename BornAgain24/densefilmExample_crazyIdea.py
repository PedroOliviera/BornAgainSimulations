...
def get_sample(approximation,radius):
    ...
    density = max_particle_density(diam_K.mean()) * 17
    print(density)
    
    for i in range(P1):
        ff = ba.SpheroidalSegment((diam_K[i]/2) * nm, height_K[i]/2 * nm, 0, 0)
        #ff = ba.Spheroid(radius * nm, radius/1.5 * nm)
        print(weight_K[i])
        particle = ba.Particle(material_P2VP, ff)
        layer_PS_Top.plugLiquid(density * weight_K[i], particle, approximation)
...
if __name__ == '__main__':
    radi = [1]
    samples = [ get_sample(ba.Random3D_PY, radius) for radius in radi]
    results = [ get_simulation(sample).simulate() for sample in samples ]
    labels = [str(radius) for radius in radi] #[20*nm, 25*nm, 27*nm, 30*nm, 33*nm]
    for label, r in zip(labels, results):
        simulationData = dac.asNpArray(r.dataArray())
        save_filename = "test_" + str(label) + "_hemiellipsoid_distribution_15deg_3D.npy"
