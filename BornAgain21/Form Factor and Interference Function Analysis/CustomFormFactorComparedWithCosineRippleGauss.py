...
def get_sample_CustomFormFactor(num_samples):
...
def get_sample_CosineRippleGauss(num_samples):
    material_PS = ba.RefractiveMaterial("PS", 2.50267703E-06, 2.46904652E-09)
    material_P2VP = ba.RefractiveMaterial("P2VP", 3E-06, 2.46904652E-09)
    material_Si_Sub = ba.RefractiveMaterial("Si Sub", 5.04383115E-06, 7.84182177E-08) #7.644e-06
    material_SiO2 = ba.RefractiveMaterial("SiO2", 4.74631315E-06, 4.16025294E-08)
...
def get_sample_HemiEllipsoid(num_samples):
    material_PS = ba.RefractiveMaterial("PS", 2.50267703E-06, 2.46904652E-09)
    material_P2VP = ba.RefractiveMaterial("P2VP", 3E-06, 2.46904652E-09)
    material_Si_Sub = ba.RefractiveMaterial("Si Sub", 5.04383115E-06, 7.84182177E-08) #7.644e-06
    material_SiO2 = ba.RefractiveMaterial("SiO2", 4.74631315E-06, 4.16025294E-08)
...
def main():
    ...
    simulation_line = g.get_simulation_line(sample_model=get_sample_CustomFormFactor(number_of_samples), detectorDistBeamtime='feb', angle_of_incidence= 0.13, center_horizontal_slice_value=0.22, center_vertical_slice_value= 0, number_slices=5,oneThread=True)
    result = simulation_line.simulate()
    simul_dat = result.extracted_field()
    final_array = simul_dat.npArray()
    save_filename = "tests_13deg_line_CustomFormFactor_comp.npy"
    np.save(save_filename, final_array)

    simulation_line = g.get_simulation_line(sample_model=get_sample_CosineRippleGauss(number_of_samples), detectorDistBeamtime='feb', angle_of_incidence= 0.13, center_horizontal_slice_value=0.22, center_vertical_slice_value= 0, number_slices=5)
    result = simulation_line.simulate()
    simul_dat = result.extracted_field()
    final_array = simul_dat.npArray()
    save_filename = "tests_13deg_line_CosineRippleGauss_comp.npy"
    np.save(save_filename, final_array)

    simulation_line = g.get_simulation_line(sample_model=get_sample_HemiEllipsoid(number_of_samples), detectorDistBeamtime='feb', angle_of_incidence= 0.13, center_horizontal_slice_value=0.22, center_vertical_slice_value= 0, number_slices=5)
    result = simulation_line.simulate()
    simul_dat = result.extracted_field()
    final_array = simul_dat.npArray()
    save_filename = "tests_13deg_line_HemiEllipsoid_comp.npy"
    np.save(save_filename, final_array)
 
    simulationDataAxes = g.get_axes_limits(result, ba.Coords_QSPACE)
    data2D_Custom = np.load("tests_13deg_line_CustomFormFactor_comp.npy")
    data2D_Ripple = np.load("tests_13deg_line_CosineRippleGauss_comp.npy")
    data2D_HemiEllipsoid = np.load("tests_13deg_line_HemiEllipsoid_comp.npy")

    vert_slice_q = 0.2

    graphing.yonedaPlot(vert_slice_q, [data2D_Custom, data2D_Ripple, data2D_HemiEllipsoid], data_axes=simulationDataAxes, data2_npArray=realData_npArray, data_axes2=realDat_axes_Feb, xmin = 0.1, xmax = 0.2) #, data2_npArray=realData_npArray, data_axes2=realDat_axes_Feb)
